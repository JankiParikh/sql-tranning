...
SELECT R.REGION_NAME, COUNT(C.REGION_ID) AS CNTS 
FROM COUNTRIES C 
JOIN REGIONS R 
ON C.REGION_ID = R.REGION_ID 
GROUP BY R.REGION_NAME 
ORDER BY COUNT(C.REGION_ID) DESC, R.REGION_NAME;

SELECT * FROM CUSTOMERS;

SELECT * FROM ORDERS;

SELECT C.NAME, COUNT(O.CUSTOMER_ID) AS CNTS 
FROM CUSTOMERS C 
INNER JOIN ORDERS O 
ON C.CUSTOMER_ID=O.CUSTOMER_ID 
GROUP BY C.NAME 
ORDER BY COUNT(O.CUSTOMER_ID) DESC, C.NAME;

SELECT C.NAME, COUNT(O.CUSTOMER_ID) AS CNTS, SUM(OI.QUANTITY*OI.UNIT_PRICE) AS TOTAL_ORDER_AMOUNT 
FROM CUSTOMERS C 
INNER JOIN ORDERS O 
ON C.CUSTOMER_ID=O.CUSTOMER_ID 
...
SELECT W.WAREHOUSE_NAME, PC.CATEGORY_NAME, SUM(I.QUANTITY) AS SUM 
FROM PRODUCTS P 
...
---CORRECT ANSWER TO Q2A 
with ordcnts as  
(select c.customer_id,count(1) cnts from customers c 
left join orders o 
on c.customer_id=o.customer_id 
   group by c.customer_id  
    order by count(1) desc 
) 
select distinct c.name ,cnts 
from orders o  
inner join 
customers c 
...
inner join 
ordcnts 
on ordcnts.customer_id=c.customer_id 
    where ordcnts.cnts=(select max(cnts) from ordcnts) 
    order by c.name;
...
select c.name ,cnts,sum(quantity*unit_price) total_order_amount 
from orders o  
inner join 
customers c 
on o.customer_id=c.customer_id 
...
on o.order_id=oi.order_id 
where ordcnts.cnts=(select max(cnts) from ordcnts) 
group by c.name ,cnts 
order by total_order_amount desc,c.name
